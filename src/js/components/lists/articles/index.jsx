...
const articles = ({ articles, articlesLoading, fetchArticles, loadingIndex }) => {
  ...
  const filterFunction = (article) => {
    ...
    if ((!!isPassingFilter) && (Array.isArray(filterTags)) && (filterTags.length !== 0)) {
      if (Array.isArray(article.tags)) {
        const { length } = filterTags;
        for (let idx = 0; idx < length; idx++) {
          if ((!!filterTags[idx]) && (article.tags.indexOf(filterTags[idx]) === -1)) {
  ...
  const handleClickRemoveCurrentSelectedFilter = () => {
    if ((!!currentSelectedFilterTag) && (!!currentSelectedFilterTag[0])) {
      setFilterTags([...filterTags.filter((tag) => tag !== currentSelectedFilterTag[0])]);
      setCurrentSelectedFilterTag([]);
    }
  };

  const renderData = () => {
    return !isLoading && Array.isArray(articles) && articles.filter(filterFunction).sort(sortFunction).map((key, index) => {
  ...
  const displayTagOptions = () => {
    return Array.isArray(availableTags) && availableTags.map((tag) => {
          ...
          <Grid container spacing={0}>
            ...
            <Grid item xs={12} sm={12} md={12} lg={5}>
              ...
              <FormControl style={{ minWidth: '224px', marginRight: '12px' }}>
                ...
                <Button
                  color='primary'
                  disabled={((!currentAvailableFilterTag) || (currentAvailableFilterTag.length === 0))}
              ...
              <FormControl style={{ minWidth: '224px' }}>
                ...
                <Button
                  color='primary'
                  disabled={((!currentSelectedFilterTag) || (currentSelectedFilterTag.length === 0))}
